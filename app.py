...
import os
from models import db, Employee, LeaveRequest ,Admin

#def create_app():
app = Flask(__name__)
app.secret_key = 'gxfdgxhtrt'  
...
# Fonction utilitaire pour ajouter un utilisateur au dictionnaire
def add_user_to_dict(username, password, role):
    users[username] = {
        'username': username,
        'password': password,
        'role': role
...
def homepage_user():
    ...
    if user:
        ...
        user_info = {
            ...
            'JrsTotal': user.jrs_total,
            'JrsRestant': user.jrs_restant
        }
    else:
        user_info = {}
...
def homepage_admin():
    ...
    if admin:
        ...
        admin_info = {
            ...
            'JrsTotal': admin.JrsTotal,
            'JrsRestant': admin.JrsRestant
        }
    else:
        admin_info = {}
...
def ajouter_employe():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            db.session.commit()
            # Ajouter l'employé au dictionnaire users
            add_user_to_dict(username, mot_de_passe, 'user')
...
def edit_employee(emp_id):
    ...
    if request.method == 'POST':
        ...
        employee.matricule = request.form['id']
        ...
        date_naiss_str = request.form['date_naiss']
        date_debut_str = request.form['date_debut']
        employee.date_naiss = datetime.strptime(date_naiss_str, '%Y-%m-%d')
        employee.date_debut = datetime.strptime(date_debut_str, '%Y-%m-%d')
